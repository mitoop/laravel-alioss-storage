...
class AliOSSAdapter extends AbstractAdapter
{
    ...
    {
        return $this->getUrlSchema().($this->customDomain ?: $this->bucket.'.'.$this->endPoint).'/'.ltrim($path, '\\/');
    }

    protected function getUrlSchema()
    {
        if ($this->urlSchema == 'http' || $this->urlSchema == 'https') {
            return $this->urlSchema.'://';
        } elseif ($this->urlSchema == 'both') {
            return '//';
        }
    ...
    public function signUrl($object, $timeout, $method)
    {
        return $this->client->signUrl($this->getBucket(), $object, $timeout, $method);
    }
